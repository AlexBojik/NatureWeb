...
export class ObjectTableComponent implements OnInit {
  ...
  applyFilter(event: Event): void {
    ...
    this.objects.forEach(obj  =>  {
      obj.checked = this.dataSource.filteredData.find(obj1 => (obj1 === obj) && obj.checked) === obj;
    });
    this.recalcCheckedCount();
  }

  update(layer: Layer): void {

    if (this.layerLoaded?.id === layer.id) {
      return;
    }
    ...
    this._objSrv.getObjects(layer.id)
      .then(objects => {
        ...
        if (layer.id !== this.layerLoaded.id) {
          return;
        }
  ...
  private recalcCheckedCount(): void {
    this._isCheckedAll = this.objects.every(o => o.checked);
    this.checkedCount = this.objects.filter(o => o.checked).length;
  }
